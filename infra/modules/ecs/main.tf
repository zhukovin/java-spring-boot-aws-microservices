...
resource "aws_security_group" "ecs_tasks" {
  ...
  # Allow inter-service calls (api-gateway → order-service, etc.)
  ingress {
    from_port = 8080
    to_port   = 8080
    protocol  = "tcp"
...
resource "aws_ecs_task_definition" "services" {
  ...
  container_definitions = jsonencode([{
    ...
    # Kafka bootstrap servers injected from Secrets Manager at task start
    secrets = [
      {
...
resource "aws_ecs_service" "services" {
  ...
  network_configuration {
    ...
    # In default VPC (all subnets public) Fargate needs a public IP to reach ECR
    assign_public_ip = true
  }

  # Only api-gateway is registered with the ALB
  ...
  # Automatically roll back on failed deployments
  # Interview note: ECS deployment circuit breaker prevents bad deploys from
  # draining all healthy tasks before rolling back.
